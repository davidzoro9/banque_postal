...
interface RefItem { code: string; libelle: string; description: string; actif: boolean; }

const MOCK: Record<string, RefItem[]> = {
  'emploi': [
...
export class DbRefListComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private moduleNav: ModuleNavService) {}
  ...
  ngOnInit(): void {
    ...
    this.route.data.subscribe(data => {
      ...
      this.type   = data['type']   ?? '';
      this.dataSource.data = MOCK[this.type] ?? [];
      this.searchQuery = '';
    });
  }
